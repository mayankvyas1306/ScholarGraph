...
class QueryRequest(BaseModel):
    ...
    filters: Optional[Dict[str, Any]] = Field(default_factory=dict,
                                              description="Optional filters like year_range, keywords")
...
class QARequest(BaseModel):
    ...
    history: Optional[List[Dict[str, str]]] = Field(default_factory=list,
                                                    description="Optional chat conversation history")
...
"""
    try:
        response = llm.complete(prompt=prompt, system=system_prompt, temperature=0.2)
    except Exception as e:
        logger.error(f"Error calling LLMClient in QA: {e}")
